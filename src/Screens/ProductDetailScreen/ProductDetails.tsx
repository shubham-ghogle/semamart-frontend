...
export default function ProductCard() {
  ...
  const [selectedPack, setSelectedPack] = useState<{ qty: number; price: number; label: string } | null>(null);
  ...
  // ✅ Build productMedia
  const productMedia: { type: "image" | "video"; src: string }[] = React.useMemo(() => {
    if (!product) {
  ...
  const variantBulkOrders: { qty: number; price: number }[] =
    Array.isArray(selectedVariant?.bulkOrders) && selectedVariant?.bulkOrders.length > 0
      ? selectedVariant.bulkOrders.slice().sort((a: any, b: any) => a.qty - b.qty)
      : [];
  ...
  const computePerPiece = (pack: { qty: number; price: number } | null) => {
    ...
    const savedPercent = orig ? Math.round(((orig - perPiece) / orig) * 100) : 0;
    return { perPiece, savedPercent: Math.max(0, savedPercent) };
  };

  const { perPiece: selectedPerPiece, savedPercent: selectedSavedPercent } = computePerPiece(selectedPack);
  ...
  useEffect(() => {
    ...
    const vb = Array.isArray(selectedVariant.bulkOrders) ? selectedVariant.bulkOrders : [];
    if (vb.length > 0) {
      if (!selectedPack || !vb.some((b: any) => b.qty === selectedPack.qty && b.price === selectedPack.price)) {
  ...
  const handleAddCart = (e: React.MouseEvent) => {
    ...
    const packQty = selectedPack?.qty ?? 1;
    const packPrice =
      selectedPack?.price ??
    ...
    const perPiece = selectedPack ? packPrice / Math.max(packQty, 1) : packPrice;
    ...
    <div className="w-full min-h-screen bg-white font-sans pt-8 pb-12 px-0 ">
      <div className="flex flex-col lg:flex-row gap-8 w-full max-w-[1600px] mx-auto box-border px-4">
        ...
        <div
          ...
          style={{ minHeight: imgSectionHeight, alignSelf: "flex-start", paddingTop: "32px", paddingBottom: "32px" }}
        >
          <ProductMediaSection
            product={product}
                ...
                {(product as any).variants.map((v: any) => (
                  <button
                    ...
                    className={`px-4 py-2 border rounded-lg text-sm transition ${
                      selectedVariant?._id === v._id ? "bg-[#1C647C] text-white" : "bg-white text-gray-700 border-gray-300"
                    }`}
                  >
        ...
        <div className="w-full lg:w-[60%] flex flex-col gap-8">
          ...
          <ProductBottomSections product={product} selectedVariant={selectedVariant} />
        </div>
      </div>
