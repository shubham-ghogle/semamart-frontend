...
import { SellerProfileButton, UserProfileButton } from "./AuthButtons";
    ...
    <header className="w-full bg-white shadow-md border-b font-inter text-xl">
      <div className="w-full flex flex-col sm:flex-row items-center h-auto sm:h-28 px-6 sm:px-10 gap-6 sm:gap-4">
        ...
        <div className="flex flex-1 items-center h-full">
              ...
              >
                ...
                {hoveredCategory &&
                  ...
                  subcategoryMap[hoveredCategory._id].length > 0 && (
                    ...
                    >
                      {subcategoryMap[hoveredCategory._id].map(
                        (sub: Subcategory) => (
                          <div
                            key={sub._id}
                            ...
                            onClick={() => {
                              navigate(
                                `/get-products-by-subcategory/${sub._id}`
                              );
                              setIsCategoryOpen(false);
                              setHoveredCategory(null);
                            }}
          ...
          {/* Search input */}
          <div
            className="flex items-center h-full relative flex-1 max-w-2xl"
            ref={containerRef}
          >
            <input
              type="text"
        ...
        <div className="flex flex-wrap sm:flex-nowrap items-center text-[11px] ml-0 sm:ml-2 gap-1 flex-shrink-0 justify-center sm:justify-start w-full sm:w-auto">
          ...
          <div className="flex items-center  font-montserrat text-[#1C647C]">
            ...
            >
              <Link
                to="/"
                className="px-5 py-3 rounded-md transition flex items-center text-lg"
              >
                        ...
                        {isLoadingSpecialties ? (
                          ...
                          specialties.map((specialty) => (
                            <li
                              ...
                              onClick={() => {
                                navigate(
                                  `/get-products-by-speciality-package/${String(
                                    specialty._id
                                  )
                                    .toLowerCase()
                                    .replace(/[^a-z0-9]+/g, "-")}`
                    ...
                    {hoveredSpecialtyId &&
                      specialtyPackageTypes[hoveredSpecialtyId] && (
                        <div className="w-[250px] p-4 overflow-y-auto bg-gray-50">
                          {specialtyPackageTypes[hoveredSpecialtyId].length ===
                          0 ? (
                            <p className="text-gray-500">
                              No package types found for this specialty.
                            </p>
                          ) : (
                            <ul>
                              {specialtyPackageTypes[hoveredSpecialtyId].map(
                                (pkg) => (
                                  <li
                                    key={pkg._id}
          ...
          {seller && (
            <SellerProfileButton sellerName={seller.firstName || ""} />
          )}
          {!user && !seller && (
            <Link
              to="/login"
              className="px-5 py-2 text-custom-blue text-lg flex gap-2 items-center rounded-full hover:bg-custom-blue hover:text-white"
            >
              <UserCircle />
              <span>Login</span>
            </Link>
          ...
          {user && (
            <button
              ...
              className="px-5 py-3 rounded-md text-[#1C647C] text-lg cursor-pointer"
            >
              Become a Seller
            </button>
          )}
