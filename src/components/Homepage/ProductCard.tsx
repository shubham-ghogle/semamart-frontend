...
export default function ProductCard({ product, mode = "default" }: Props) {
  ...
  const isAvailableToOrder =
    (product as any)?.isAvailableToOrder !== false &&
    (product as any)?.visibilityByAdmin !== false &&
    (product as any)?.visibilityBySeller !== false;
  ...
  const stock: number = Number(variant?.stock ?? 0);
  ...
  const inWishlist = wishlist.some(
    (w: any) =>
      (w.product && (w.product as any)._id === productId) ||
  ...
  const handleAddCart = (e?: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (inWishlist) {
      removeFromWishlist(productId, variant._id);
    }
  ...
  const getCategoryLabel = () => {
    ...
    if (typeof cat === "string") {
      return categoriesMap[cat] ?? cat ?? "General";
    }
  ...
  React.useEffect(() => {
    ...
    const sync = () => {
      setIsTicked(
        getMedicopList().some((x) => x.productId === productId)
      );
    };
        ...
        >
          ...
          {!isMedicopMode && (
            <div className="flex items-center gap-1 mt-1 md:flex" aria-hidden>
              <StarIcons
                stars={(product as any).avgRating ?? 0}
          ...
          {!isMedicopMode && (
            <div className="flex items-center gap-2 mt-1">
              <span className="text-base md:text-lg font-semibold text-gray-900">
                Rs.{" "}
                {(discountPrice ?? originalPrice ?? 0).toLocaleString(
                  "en-IN",
                  {
