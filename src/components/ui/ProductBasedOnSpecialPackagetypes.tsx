...
export default function ProductBasedOnSpecialPackagetypes() {
  ...
  // ✅ SAME FETCH STRUCTURE AS CATEGORY (only API changes)
  useEffect(() => {
    const fetchProducts = async () => {
      if (!id) {
        setLoading(false);
        setError("Invalid speciality package type ID");
        return;
      }
      try {
        const res = await fetch(
          `${API_URL}product/get-products-by-speciality-package-type/${id}`
        );
        ...
        if (Array.isArray(data)) {
          // ✅ SAME manufacturer normalization as category
  ...
  // ✅ SAME filter & sort logic as category
...
md:relative md:translate-x-0 md:w-64 md:z-1 border-r p-5 ${
        ...
        <main className="flex-1 p-4 md:p-8">
          ...
          {/* ✅ SAME card as category → images WILL work */}
