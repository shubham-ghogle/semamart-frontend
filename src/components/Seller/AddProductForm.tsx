                    ...
                    <FormControl>
                      <>
                        {form.getValues("category").map((el) => (
                          <Input value={el.name} key={"cat-" + el.val} readOnly className="w-full mb-2" />
                        ))}
                        <Autocomplete
                          ...
                          setValue={(value) => {
                            const catName =
                              categoryDropDownList.find((el) => el.value === value)?.label || "";
                    ...
                    <FormControl>
                      <>
                        {form.getValues("subCategory").map((el, i) => (
                          <article className="grid grid-cols-[1fr_50px] items-center gap-3 mb-2" key={"subcat-" + el.val}>
                            <Input value={el.name} readOnly className="w-full" />
                            <Button key={el.val} type="button" size="sm" variant="destructive" onClick={() => deleteCategory(i)}>
                              <IoRemoveCircle />
                            </Button>
                          </article>
                        ))}
                        <article className="grid grid-cols-[1fr_50px] items-center gap-3">
                          ...
                          />
                          <Button variant="secondary" size="sm" type="button" onClick={addCaategory}>
                            Add
                          </Button>
                        </article>
                      </>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      ...
                      <SelectContent>
                        <SelectItem value="Non-Replaceable">Non-Replaceable</SelectItem>
                        <SelectItem value="Replaceable">Replaceable</SelectItem>
                      </SelectContent>
                    </Select>
                    <FormMessage />
                  </FormItem>
              ...
              <div className="space-y-2">
                {upsellFields.map((_field, index) => (
                  <div key={index} className="flex gap-2">
                    <Input {...form.register(`upsells.${index}`)} placeholder="Enter product URL" className="w-full" />
                    <Button type="button" variant="ghost" size="icon" onClick={() => removeUpsell(index)}>
                      ✕
                    </Button>
                  </div>
                ))}
                <Button type="button" variant="outline" onClick={() => addUpsell("")} disabled={upsellFields.length >= 5}>
                  + Add Upsell
                </Button>
              </div>
              <FormMessage />
            ...
            <FormItem>
              ...
              <div className="space-y-2">
                {crossFields.map((_field, index) => (
                  <div key={index} className="flex gap-2">
                    <Input {...form.register(`crosssells.${index}`)} placeholder="Enter product URL" className="w-full" />
                    <Button type="button" variant="ghost" size="icon" onClick={() => crossRemove(index)}>
                      ✕
                    </Button>
                  </div>
                ))}
                <Button type="button" variant="outline" onClick={() => crossAppend("")} disabled={crossFields.length >= 5}>
                  + Add Cross-sell
                </Button>
              </div>
              <FormMessage />
                      ...
                      {showManuDropdown && manufacturerList?.length > 0 && (
                        <ul className="border p-2 max-h-48 overflow-y-auto absolute bg-white shadow inset-x-0 z-[900] sm:max-w-md">
                          {manufacturerList.map((m: any) => (
                            <li key={m._id} className="py-1 border-b last:border-none">
                              <button
                                type="button"
                                className="w-full text-left"
                                onClick={(e) => {
                                  e.preventDefault();
                                  form.setValue("manufacturerName", m.manufacturerName);
                    ...
                    <>
                      {field.value?.map((e, i) => (
                        <div className="flex gap-3 mb-3" key={i}>
                          <Input value={Object.keys(e)[0]} readOnly className="w-2/5" />
                          <Input value={Object.values(e)[0]} readOnly className="w-3/5" />
                          <Button type="button" variant="destructive" onClick={() => { removeAttri(i); }}>
                            <IoRemoveCircle />
                          </Button>
                        </div>
                      ))}
                      <div className="flex gap-3">
                        <Input
                          ...
                          onChange={(e) => {
                            setCurrentAttri((p) => ({ ...p, key: e.target.value }));
                          }}
                          className="w-1/2"
                        />
                        <Input
                          ...
                          onChange={(e) => {
                            setCurrentAttri((p) => ({ ...p, val: e.target.value }));
                          }}
                          className="w-1/2"
                        />
                        <Button type="button" variant="ghost" onClick={() => addAddtri({ [currentAttri.key]: currentAttri.val })}>
                          Add
                        </Button>
                      </div>
                    </>
              ...
              <div className="my-6 space-y-4">
                <div className="space-y-3">
                  {variantFields.map((field, index) => (
                    <div key={field.id} className="bg-gray-50 p-3 rounded-md sm:p-4">
                      <AddProductFormVariants
                        ...
                        thumbnailError={(form.formState.errors as any).thumbnail}
                  ...
                  {isMultiVariant && (
                    <article className="flex justify-end">
                      <Button type="button" size="icon" variant="outline" className="bg-green-100 cursor-pointer" onClick={addVariant}>
                ...
                <div>
                  ...
                  <div className="flex gap-3 flex-wrap mt-2">
                    {Array.from({ length: 4 }).map((_, index) => (
                      ...
                      >
                        <label htmlFor={`uploadImage-${index}`} className="cursor-pointer w-full h-full block">
                          {images[index] ? (
                            <div className="h-full w-full relative">
                              <img src={URL.createObjectURL(images[index])} alt={`Image-${index + 1}`} className="h-full w-full object-cover" />
                              <Button
                                size="icon"
                                variant="destructive"
                        ...
                        <input type="file" id={`uploadImage-${index}`} className="hidden" onChange={(e) => handleImageChange(e, index)} />
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="border border-gray-300 h-[90px] w-[90px] sm:h-[120px] sm:w-[120px] flex items-center justify-center rounded-md cursor-pointer mt-2 relative overflow-hidden">
                    <label htmlFor="uploadShortVideo" className="cursor-pointer w-full h-full grid place-items-center">
                      {shortVideo ? (
                        <div className="h-full w-full relative">
                          <video controls={true} src={URL.createObjectURL(shortVideo)} className="h-full w-full object-cover" />
                          <Button
                            onClick={(e) => {
                              e.preventDefault();
                              e.stopPropagation();
